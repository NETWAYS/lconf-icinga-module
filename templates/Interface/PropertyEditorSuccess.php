	...
	var gridPanel = Ext.extend(Ext.grid.GridPanel,{
		...
		ds :  new Ext.data.JsonStore({
			proxy: new Ext.data.HttpProxy({
				api: {
					read :'<?php echo $ro->gen("LDAP.data.propertyProvider");?>',
					create: String.format('<?php echo $ro->gen("LDAP.data.modifyEntry");?>{0}',"propertyCreate"),
					update: String.format('<?php echo $ro->gen("LDAP.data.modifyEntry");?>{0}',"alter"),
					destroy: String.format('<?php echo $ro->gen("LDAP.data.modifyEntry");?>{0}',"propertyDelete"),
				},
			}),
			autoSave: false,
